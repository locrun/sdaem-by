...
const data = [
  {
    ...
    btnName: "+ Разместить объявление",
    imgPath: client,
    styleButton: classes.placeBtn,
    ...
    subtitle: {
      paragraph: (
        <div>
          ...
          <strong> вверх первой страницы </strong>каталога, они разместятся
          сразу после платных объявлений до тех пор, пока другой пользователь не
          повторит процедуру.
        </div>
      ),
    },
    btnName: "Узнать стоимость услуги",
    imgPath: raise,
    styleButton: classes.priceBtn,
    ...
    subtitle: {
      paragraph: (
        <div>
          ...
          <p>
            Gold объявления <strong>перемещаются</strong>
            <br />
            <strong> каждые 5 мин </strong>на 1 позицию, что делает размещение
            одинаковым для всех.
          </p>
        </div>
      ),
          ...
          {data.map((card) => (
            <div key={card.id} className={cn(classes.card, card.className)}>
              ...
              <div className={classes.cardSubtitle}>
                {card.subtitle.paragraph}
              </div>
              <Button className={card.styleButton}>{card.btnName}</Button>
            </div>
          ))}
        </div>
      </div>
