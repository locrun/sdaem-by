...
export const useCottagesFilter = () => {
  const { cottages, cottagesData } = useAppSelector((state) => state.cottages);
  const [filteredCottagesData, setFilteredCottagesData] = useState<ICottages[]>(
    []
  );
  ...
  useEffect(() => {
    ...
    setFilteredCottagesData(filteredData);
  }, [cottages, cottagesData]);

  return {
    filteredCottagesData,
  };
};
